...
func InitMathOperationsGame(c GameConfig, players []models.Player, fM func(models.Message), bM func([]byte)) MathOperations {
	m := MathOperations{
		config:           c,
		broadcastMessage: bM,
		forwardMessage:   fM,
...
type MathQuestion struct {
	Question      string
	Answers       []int
	CorrectAnswer int `json:",omitempty"`
}

func (m MathOperations) generateAdditionQuestion() MathQuestion {
	numberOne := rand.Intn(1000) - 500
	numberTwo := rand.Intn(1000) - 500
	...
	for range 3 {
		answers = append(answers, result+rand.Intn(100)-50)
	}
